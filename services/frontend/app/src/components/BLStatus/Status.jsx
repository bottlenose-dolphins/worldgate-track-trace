...
export default function Status() {
  ...
  const { cordsLat } = location.state;
  const { cordsLong } = location.state;
  const { destinationLat } = location.state;
  const { destinationLong } = location.state;
  ...
  return (
    <div className="bg-gradient-to-r from-white via-sky-100 to-sky-200 w-screen">
      ...
      <VesselView cordsLat={cordsLat} cordsLong={cordsLong} destinationLat={destinationLat} destinationLong={destinationLong} />
