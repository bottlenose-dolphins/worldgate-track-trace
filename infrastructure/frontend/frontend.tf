resource "aws_ecs_service" "tracktrace_default_subnet_fe_service" {
    ...
    cluster         = "${data.aws_ecs_cluster.tracktrace_cluster.id}"             # Referencing our created Cluster
    ...
    load_balancer {
    target_group_arn = "${aws_lb_target_group.target_group_fe.arn}" # Referencing our target group
...
resource "aws_ecs_task_definition" "tracktrace_test_task2" {
    ...
    [
        ...
        "image": "283879969377.dkr.ecr.ap-southeast-1.amazonaws.com/tracktrace_repo:fe",
        "essential": true,
        "portMappings": [
            {
            "containerPort": 80,
    ...
    execution_role_arn       = "${data.aws_iam_role.ecsTaskExecutionRole2.arn}"
}
