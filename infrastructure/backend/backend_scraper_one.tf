resource "aws_cloudwatch_log_group" "tracktrace_scraper_one_log" {
    name = "tracktrace_scraper_one_log"
}
...
resource "aws_ecs_task_definition" "tracktrace_scraper_one" {
    ...
    [
        ...
        "image": "283879969377.dkr.ecr.ap-southeast-1.amazonaws.com/tracktrace_repo:scraper_One",
        "essential": true,
        "portMappings": [
            {
            "containerPort": 80,
        ...
        "logConfiguration": {
          "logDriver": "awslogs",
          "options": {
            "awslogs-group": "tracktrace_scraper_one_log",
