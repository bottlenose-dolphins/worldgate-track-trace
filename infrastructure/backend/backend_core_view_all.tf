resource "aws_cloudwatch_log_group" "tracktrace_core_view_all_log" {
    name = "tracktrace_core_view_all_log"
}

resource "aws_ecs_task_definition" "tracktrace_core_view_all" {
    family                   = "tracktrace_core_view_all" 
    container_definitions    = <<DEFINITION
    [
        {
        "name": "tracktrace_core_view_all",
        "image": "283879969377.dkr.ecr.ap-southeast-1.amazonaws.com/tracktrace_repo:core_view-all",
        "essential": true,
        "portMappings": [
            {
            "containerPort": 5010,
        ...
        "logConfiguration": {
          "logDriver": "awslogs",
          "options": {
            "awslogs-group": "tracktrace_core_view_all_log",
...
resource "aws_ecs_service" "tracktrace_core_view_all_service" {
    name            = "tracktrace_core_view_all_service"                             # Naming our first service
    cluster         = "${aws_ecs_cluster.tracktrace_cluster.id}"             # Referencing our created Cluster
    task_definition = "${aws_ecs_task_definition.tracktrace_core_view_all.arn}" # Referencing the task our service will spin up
    launch_type     = "FARGATE"
    ...
    service_registries {
    registry_arn = "${aws_service_discovery_service.core_view_all.arn}"
    }
}
...
resource "aws_lb_listener" "listener_core_view_all" {
